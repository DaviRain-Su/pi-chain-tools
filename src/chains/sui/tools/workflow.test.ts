...
describe("w3rt_run_sui_workflow_v0", () => {
	...
	it("parses LP remove intentText with shorthand min A/min B", async () => {
		const tool = getTool();
		const result = await tool.execute("wf5d-remove-shorthand", {
			runId: "wf-sui-05d-remove-shorthand",
			runMode: "analysis",
			network: "mainnet",
			intentText:
				"remove liquidity pool: 0xabc position: 0xdef SUI/USDC tick: -5 to 5 deltaLiquidity: 1000 min a: 1.2 min b: 2.5",
		});
	...
	it("parses LP remove intentText with natural-language min output phrase", async () => {
		const tool = getTool();
		const result = await tool.execute("wf5d-remove-nl", {
			runId: "wf-sui-05d-remove-nl",
			runMode: "analysis",
			network: "mainnet",
			intentText:
				"remove liquidity pool: 0xabc position: 0xdef SUI/USDC tick: -5 to 5 deltaLiquidity: 1000 最少拿到 1.2 SUI 2.5 USDC",
		});
