...
- BSC mode supports quote+minOut planning for `rebalance_usdt_to_usdce_txn` (`chain=bsc`), and can execute in two built-in adapter modes:
  ...
  - `POST /api/bsc/yield/execute` now returns `postActionArtifact` (protocol-agnostic `bsc_post_action_supply@v1`) and `postActionReconciliation` (routed via artifact type/version), and persists both in ACP job history for adapter-agnostic reconciliation
