...
- `kaspa_signTransferTransaction`：新增签名承接工具，支持在请求对象/原始交易基础上附加或覆盖签名，返回可执行 `request` 与 `requestHash`。
...
- 输出：`request`（含签名数组）与 `signatureEncoding`，以及 `requestHash`/`unsignedRequestHash`
