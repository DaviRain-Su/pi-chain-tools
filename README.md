...
- `trade stale requote`: workflow trade supports `requoteStaleOrders=true` + stale filters (`maxAgeMinutes` / `maxFillRatio`) to run cancel-stale then repost in execute mode; optional requote limits `requoteMinIntervalSeconds` / `requoteMaxAttempts`
