...
- `trade stale requote`: workflow trade supports `requoteStaleOrders=true` + stale filters (`maxAgeMinutes` / `maxFillRatio`) to run cancel-stale then repost in execute mode; supports pricing strategy `requotePriceStrategy=aggressive|passive|follow_mid` and limits `requoteMinIntervalSeconds` / `requoteMaxAttempts`
