...
export function validateStrategySemanticV1(normalizedDsl, policy = {}) {
	...
	if (Number.isFinite(minRebalanceUsd) && minRebalanceUsd > 0) {
		if (Number(dsl.risk?.maxAmountUsd || 0) < minRebalanceUsd) {
			errors.push(
				`risk.maxAmountUsd: (${dsl.risk?.maxAmountUsd}) must be >= policy.constraints.minRebalanceUsd (${minRebalanceUsd})`,
			);
		}
	}
	...
	if (
		...
		errors.push(
			`risk.dailyRunLimit: (${dsl.risk?.dailyRunLimit}) exceeds policy.constraints.maxDailyRebalanceRuns (${maxDailyRebalanceRuns})`,
		);
	}
