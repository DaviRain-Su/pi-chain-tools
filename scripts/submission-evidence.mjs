...
const EXECUTION_PROOFS_DIR = path.join(ROOT, "docs", "execution-proofs");
...
function findLatestProofDocs(limit = 5) {
	try {
		const dayDirs = readdirSync(EXECUTION_PROOFS_DIR)
			.filter((name) => /^\d{4}-\d{2}-\d{2}$/.test(name))
			.sort()
			.reverse();
		const output = [];
		for (const day of dayDirs) {
			const dayDir = path.join(EXECUTION_PROOFS_DIR, day);
...
function renderEvidence() {
	...
	const latestProofs = findLatestProofDocs();
	...
	return [
		...
		...(latestProofs.length > 0
			? latestProofs.map((p) => `- ${p}`)
			: ["- (not found)", "- Generate with: `npm run execute:proof`"]),
		"",
		"---",
